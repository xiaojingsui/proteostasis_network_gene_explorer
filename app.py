...
def load_data():
    ...
    df = pd.read_excel(
        file_path,
        sheet_name="Proteostasis_Network_2024_0414"
    )
...
st.text_input(
    "",
    placeholder="Search by Gene, Branch, Class, Group, Type, Subtype, or UniProt ID...",
...
with c1:
    st.markdown(
        "<p style='text-align:right; font-size: 18px; color: #006064; padding-top: 5px;'>Try searching for:</p>",
        unsafe_allow_html=True
    )
with c2:
...
# --------------------------------------------------
# 6. Search Logic + UniProt ID Linking
# --------------------------------------------------
...
if query:
    ...
    if not results.empty:
        ...
        # UniProt ID → direct UniProt entry
        def uniprot_link(uid):
            if pd.isna(uid) or uid == "":
                return ""
            return f'<a href="https://www.uniprot.org/uniprotkb/{uid}" target="_blank">{uid}</a>'

        results["UniProt"] = results["UniProt ID"].apply(uniprot_link)
